...
from src.db_models import RawSgMrtStationExits
...
def normalize_record(record: Dict[str, Any], dataset_name: str = 'sg_mrt_station_exits', ingested_at: str = None) -> RawSgMrtStationExits:
    # --- Geo Extraction ---
    ...
    return RawSgMrtStationExits(
        ...
        estmtCnstrnCmcmnt=record.get('ESTMT_CNSTRN_CMCMNT'),
        estmtCnstrnCmpltn=record.get('ESTMT_CNSTRN_CMPLTN'),
        cnstrnCmcmnt=record.get('CNSTRN_CMCMNT'),
        implmtOrg=record.get('IMPLMT_ORG'),
        qsmContact=record.get('QSM_CONTACT'),
        ctrctrName=record.get('CTRCTR_NAME'),
        ctrctrCntct=record.get('CTRCTR_CNTCT'),
